...
public class ProductsRepository extends BaseRepository {
    ...
    public static void save(Product product) throws SQLException {
        ...
        try (var conn = dataSource.getConnection();
                var preparedStatements = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            preparedStatements.setString(1, product.getTitle());
            preparedStatements.setInt(2, product.getPrice());
            preparedStatements.executeUpdate();
            var generatedKeys = preparedStatements.getGeneratedKeys();
